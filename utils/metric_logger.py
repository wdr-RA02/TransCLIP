...
class TensorboardLogger(MetricLogger):
    def __init__(self,
        ...
        # groups that does not need to log avg in tboard
        self.no_avg_grps = no_avg_grps
        if is_main_process():
        ...
        if self.tb_logger:
            for group_name, values in self.meters.items():
                for name, meter in values.items():
                    ...
                    if group_name not in self.no_avg_grps:
                        # eval group does not need avg
